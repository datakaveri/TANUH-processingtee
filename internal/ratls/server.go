...
// Server holds the Processing TEE RA-TLS server-side state.
type Server struct {
	audience string
	tlsCert  tls.Certificate
}

// NewServer initialises the RA-TLS server with a fresh self-signed cert.
// The cert's CA is irrelevant — clients authenticate the session via the
// attestation token's EKM channel binding, not the certificate chain.
...
// TLSConfig returns the tls.Config for the HTTPS server.
...
func (s *Server) ConnectHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewEncoder(w).Encode(AttestationBundle{OIDCToken: oidcToken}); err != nil {
			log.Printf("ratls/server: encode bundle: %v", err)
		}
		log.Printf("ratls/server: RA-TLS bundle sent to %s", r.RemoteAddr)
	})
}
...
func generateSelfSignedCert() (tls.Certificate, error) {
	...
	tmpl := &x509.Certificate{
		...
		Subject:      pkix.Name{Organization: []string{"TANUH-Processing-TEE-RATLS"}},
